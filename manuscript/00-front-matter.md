...
It is not a prompt-engineering guide. You will not find phrasings that coax better output from a particular model, or tricks that stop working after an update. A specification is an engineering artifact that must outlive the model that reads it, and that constraint rules out everything clever.

It is not a vendor manual. I deliberately name no model, no vendor, no editor, and no hosted product anywhere in these pages. I do name open standards and tools that have stable public definitions, because a document that depends on a product is a document with an expiration date.
